...
const getEmptyForm = () => ({
  entry_type: 'deposit', amount: '', date: getTodayInputValue(), reference_id: '', note: '',
});

/* ── Type helpers ── */
const INFLOW_TYPES  = ['deposit', 'interest', 'transfer_in'];
const OUTFLOW_TYPES = ['withdrawal', 'charge', 'transfer_out'];
...
export default function BankEntriesPage() {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    } catch { setError('Bank entries load nahi ho paayi.'); }
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      if (!q) return true;
      return [e.entry_type, e.note, e.reference_id].join(' ').toLowerCase().includes(q);
    });
  }, [entries, search, typeFilter]);

  const totalInflow   = filteredEntries.filter((e) => INFLOW_TYPES.includes(e.entry_type)).reduce((s, e) => s + Number(e.amount || 0), 0);
  const totalOutflow  = filteredEntries.filter((e) => OUTFLOW_TYPES.includes(e.entry_type)).reduce((s, e) => s + Number(e.amount || 0), 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl(endpoint), {
        ...
        body: JSON.stringify({ entry_type: form.entry_type, amount: Number(form.amount), date: form.date, reference_id: form.reference_id, note: form.note }),
      });
      if (res.status === 401) { router.push('/login'); return; }
      const data = await res.json();
  ...
  const startEdit = (entry) => {
    ...
    setForm({ entry_type: entry.entry_type || 'deposit', amount: String(entry.amount || ''), date: new Date(entry.date || entry.createdAt).toISOString().slice(0, 10), reference_id: entry.reference_id || '', note: entry.note || '' });
    ...
    <Layout>
      <div className="max-w-2xl mx-auto px-3 sm:px-4 pt-4 pb-28 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            { label: 'Total Inflow',  value: `₹${fmtShort(totalInflow)}`,          sub: 'deposit, interest, in',              bg: 'bg-emerald-50 border-emerald-200', vc: 'text-emerald-700' },
            { label: 'Total Outflow', value: `₹${fmtShort(totalOutflow)}`,         sub: 'withdrawal, charge, out',            bg: 'bg-rose-50 border-rose-200',       vc: 'text-rose-700'    },
            { label: 'Net Movement',  value: `₹${fmtShort(Math.abs(netMovement))}`, sub: netMovement >= 0 ? 'net positive' : 'net outflow', bg: 'bg-blue-50 border-blue-200', vc: netMovement >= 0 ? 'text-blue-700' : 'text-rose-700' },
          ].map((k) => (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-3">
            {/* Entry type pills */}
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Entry Type</p>
              <div className="grid grid-cols-3 gap-2">
                {ENTRY_TYPES.map((type) => {
                  const isInflow = INFLOW_TYPES.includes(type);
                  return (
                    <button key={type} type="button" onClick={() => setForm((c) => ({ ...c, entry_type: type }))}
                      className={`flex items-center gap-1.5 px-2.5 py-2.5 rounded-xl border text-[11px] font-bold transition-all ${
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-[13px] font-bold text-slate-600">No bank entries found</p>
              <p className="text-[11px] text-slate-400 mt-1">Filter बदलो या नई entry add करो</p>
            </div>
          ) : (
