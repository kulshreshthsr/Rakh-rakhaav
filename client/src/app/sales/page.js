      ...
      <div className={`fixed inset-0 z-[70] transition-opacity duration-300 ${showModal ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}>
        ...
        <aside className={`absolute inset-x-0 bottom-0 top-14 flex max-h-[calc(100vh-56px)] flex-col rounded-t-2xl bg-white shadow-xl transition-transform duration-300 md:inset-y-0 md:right-0 md:left-auto md:top-0 md:w-[420px] md:max-h-screen md:rounded-none ${showModal ? 'translate-y-0 md:translate-x-0' : 'translate-y-full md:translate-x-full'}`}>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-4 py-3 md:px-6 md:py-4">
            ...
            {form.payment_type === 'credit' ? <section className="rounded-2xl border border-red-200 bg-red-50/60 p-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wider text-red-500">Advance Payment</p><input ref={amountPaidInputRef} className="h-10 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm" type="number" step="0.01" min="0" placeholder={`Advance payment (max ₹${fmt(billTotals.total)})`} value={form.amount_paid} onChange={(e) => updateForm({ amount_paid: e.target.value })} /><div className="mt-2 flex gap-1"><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: '0' })}>0</button><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: String((billTotals.total / 2).toFixed(2)) })}>50%</button><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: String(billTotals.total.toFixed(2)) })}>Full</button></div><div className="mt-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">Balance Due: ₹{fmt(balanceDue)}</div></section> : null}
