    ...
    <Layout>
      ...
      <div className={`fixed inset-0 z-[70] transition-opacity duration-300 ${showModal ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}>
        ...
        <aside className={`sales-entry-modal absolute inset-x-0 bottom-0 top-14 flex max-h-[calc(100vh-56px)] flex-col rounded-t-2xl bg-white shadow-xl transition-transform duration-300 md:inset-y-0 md:right-0 md:left-auto md:top-0 md:w-[420px] md:max-h-screen md:rounded-none ${showModal ? 'translate-y-0 md:translate-x-0' : 'translate-y-full md:translate-x-full'}`}>
          <div className="sticky top-0 z-20 border-b border-gray-200 bg-white px-4 py-3 md:px-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className={`entry-mode-switch mt-2 inline-flex h-10 items-center rounded-full border px-1 ${form.payment_type === 'credit' ? 'border-red-300 bg-red-50' : 'border-emerald-300 bg-emerald-50'}`}>
                  <button type="button" onClick={() => { updateForm({ payment_type: 'cash', amount_paid: '' }); setShowCustomerInfo(false); }} className={`h-8 rounded-full px-3 text-xs font-semibold sm:h-9 sm:px-4 sm:text-sm ${form.payment_type === 'cash' ? 'bg-emerald-500 text-white' : 'text-slate-600'}`}>कैश</button>
                  <button type="button" onClick={() => { updateForm({ payment_type: 'credit' }); setShowCustomerInfo(true); }} className={`h-8 rounded-full px-3 text-xs font-semibold sm:h-9 sm:px-4 sm:text-sm ${form.payment_type === 'credit' ? 'bg-red-500 text-white' : 'text-slate-600'}`}>उधार</button>
                </div>
              </div>
          ...
          <div className="sale-entry-form entry-form-shell flex-1 space-y-4 overflow-y-auto px-4 py-3 md:px-6 md:py-4">
            ...
            <section className={`entry-form-section ${form.payment_type === 'credit' ? 'entry-form-section--danger' : 'entry-form-section--cool'}`}>
              <div className="entry-section-header"><div><p className="entry-section-title">Customer</p><p className="entry-section-subtitle">Customer details ko clean aur highlighted tarah se fill karein.</p></div><span className={`entry-section-chip ${form.payment_type === 'credit' ? 'entry-section-chip--danger' : ''}`}>{form.payment_type === 'credit' ? 'Required' : 'Optional'}</span></div>
              {form.payment_type === 'cash' ? <button type="button" onClick={() => setShowCustomerInfo((value) => !value)} className="flex w-full items-center justify-between rounded-2xl border border-sky-100 bg-white/80 px-3 py-3 text-left shadow-sm"><div><p className="text-sm font-semibold text-slate-800">Customer Info</p><p className="mt-1 text-xs text-slate-500">{customerSummary}</p></div><span className="text-sm font-semibold text-sky-700">{customerInfoVisible ? '▴' : '▾'}</span></button> : null}
              {customerInfoVisible ? <><div ref={customerComboRef} className="mt-3 grid gap-2 md:grid-cols-2"><div className="relative"><input className="h-11 w-full rounded-xl border border-gray-200 px-3 text-sm" placeholder="Customer name" ref={buyerNameInputRef} value={customerQuery} onChange={(e) => { setCustomerQuery(e.target.value); updateForm({ buyer_name: e.target.value }); setShowCustomerSuggestions(true); }} onFocus={() => setShowCustomerSuggestions(true)} required={form.payment_type === 'credit'} />{showCustomerSuggestions && filteredPastCustomers.length > 0 ? <div className="absolute z-20 mt-1 max-h-44 w-full overflow-y-auto rounded-xl border border-gray-200 bg-white py-1 shadow-lg">{filteredPastCustomers.map((customer) => <button key={`${customer.phone}-${customer.name}`} type="button" onClick={() => selectPastCustomer(customer)} className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-gray-50"><span>{customer.name}</span><span className="text-xs text-slate-400">{customer.phone}</span></button>)}</div> : null}</div><input className="h-11 rounded-xl border border-gray-200 px-3 text-sm" placeholder="Phone number" value={form.buyer_phone} onChange={(e) => updateForm({ buyer_phone: e.target.value })} /></div><button type="button" onClick={() => setShowMoreCustomerDetails((v) => !v)} className="mt-2 text-xs font-semibold text-slate-500">More Details {showMoreCustomerDetails ? '▴' : '▾'}</button>{showMoreCustomerDetails ? <div className="mt-2 space-y-2"><input className="h-11 w-full rounded-xl border border-gray-200 px-3 text-sm" placeholder="GSTIN" value={form.buyer_gstin} maxLength={GSTIN_LENGTH} onChange={(e) => handleBuyerGstinChange(e.target.value)} onBlur={() => setGstinTouched(true)} />{showGstinError ? <p className="text-xs text-red-600">Invalid GSTIN format</p> : null}<select className="h-11 w-full rounded-xl border border-gray-200 px-3 text-sm" value={form.buyer_state} onChange={(e) => updateForm({ buyer_state: e.target.value })}><option value="">Select State/UT</option><optgroup label=" States ">{STATES.map((s) => <option key={s} value={s}>{s}</option>)}</optgroup><optgroup label=" Union Territories ">{UTS.map((s) => <option key={s} value={s}>{s}</option>)}</optgroup></select><input className="h-11 w-full rounded-xl border border-gray-200 px-3 text-sm" placeholder="Address" value={form.buyer_address} onChange={(e) => updateForm({ buyer_address: e.target.value })} /></div> : null}</> : null}
            </section>
            <section className="entry-form-section entry-form-section--cool"><div className="entry-section-header"><div><p className="entry-section-title">Items</p><p className="entry-section-subtitle">Product, qty aur price ab zyada highlighted dikhengi.</p></div><button type="button" onClick={() => setShowBarcodeScanner(true)} className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-xs font-semibold text-cyan-700">Scan Barcode</button></div><div className="space-y-2">{items.map((item, index) => { const g = rowGST(item); const prod = products.find((p) => p._id === item.product_id); return <div key={index} className="entry-item-card"><div className="mb-2 flex justify-end gap-1"><button type="button" className="rounded-full border border-gray-200 px-2 py-0.5 text-xs" onClick={() => duplicateItem(index)}>⧉</button>{items.length > 1 ? <button type="button" className="rounded-full border border-red-200 px-2 py-0.5 text-xs text-red-600" onClick={() => removeItem(index)}>×</button> : null}</div><SearchableProductSelect products={products} value={item.product_id} onChange={(id) => updateItem(index, 'product_id', id)} onSelectProduct={(product) => handleProductSelect(index, product)} placeholder="Select product" searchPlaceholder="Search name, barcode or HSN" />{prod ? <span className="mt-1 inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">Stock: {prod.quantity || 0}</span> : null}<div className="mt-2 grid gap-2 sm:grid-cols-2"><div><div className="flex h-8 items-center rounded-lg border border-gray-200"><button type="button" className="h-8 w-8 text-sm" onClick={() => updateItemQuantityBy(index, -1)}>-</button><input className="h-8 w-full border-x border-gray-200 text-center text-sm outline-none" type="number" min="1" max={prod ? prod.quantity : undefined} value={item.quantity} onChange={(e) => updateItem(index, 'quantity', e.target.value)} /><button type="button" className="h-8 w-8 text-sm" onClick={() => updateItemQuantityBy(index, 1)}>+</button></div><div className="mt-1 flex gap-1">{QUICK_QUANTITY_OPTIONS.map((quantity) => <button key={quantity} type="button" className={`rounded-full border px-2 py-0.5 text-[10px] ${Number(item.quantity) === quantity ? 'border-teal-600 bg-teal-600 text-white' : 'border-gray-200 text-slate-500'}`} onClick={() => applyQuickQuantity(index, quantity)}>{quantity}</button>)}</div></div><input className="h-8 rounded-lg border border-gray-200 px-2 text-sm" type="number" step="0.01" value={item.price_per_unit} onChange={(e) => updateItem(index, 'price_per_unit', e.target.value)} placeholder="Price" /></div>{g ? <p className="mt-1 text-xs text-slate-500">₹{fmt(g.taxable)} + ₹{fmt(g.gst)} GST = ₹{fmt(g.total)}</p> : null}</div>; })}<button type="button" onClick={addItem} className="entry-add-button">+ Add Item</button><p className="text-[10px] text-slate-400">Alt+A on keyboard</p></div></section>
            {form.payment_type === 'credit' ? <section className="entry-form-section entry-form-section--danger"><div className="entry-section-header"><div><p className="entry-section-title">Advance Payment</p><p className="entry-section-subtitle">Agar kuch payment abhi mila hai to yahan enter karein.</p></div></div><input ref={amountPaidInputRef} className="h-10 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm" type="number" step="0.01" min="0" placeholder={`Advance payment (max ₹${fmt(billTotals.total)})`} value={form.amount_paid} onChange={(e) => updateForm({ amount_paid: e.target.value })} /><div className="mt-2 flex gap-1"><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: '0' })}>0</button><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: String((billTotals.total / 2).toFixed(2)) })}>50%</button><button type="button" className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-[10px]" onClick={() => updateForm({ amount_paid: String(billTotals.total.toFixed(2)) })}>Full</button></div><div className="entry-summary-card entry-summary-card--danger mt-2 px-3 py-2 text-xs font-semibold text-red-700">Balance Due: ₹{fmt(balanceDue)}</div></section> : null}
            <div className="entry-summary-card sticky bottom-0 px-3 py-3"><p className="text-xs text-slate-600">Taxable: ₹{fmt(billTotals.taxable)} | GST: ₹{fmt(billTotals.gst)}</p><p className="text-lg font-bold">Total: ₹{fmt(billTotals.total)}</p><p className="text-xs text-slate-500">Round Off: {roundedBill.roundOff >= 0 ? '+' : ''}₹{fmt(roundedBill.roundOff)}</p>{form.payment_type === 'credit' ? <p className="text-xs font-semibold text-red-600">Balance Due: ₹{fmt(balanceDue)}</p> : null}</div>
          </div>
          <div className="sticky bottom-0 border-t border-gray-200 bg-white px-4 py-3 md:px-6"><div className="flex gap-2"><button type="button" onClick={handleSubmit} className="entry-submit-button h-11 flex-1 rounded-xl px-4 text-sm font-semibold text-white disabled:opacity-60" disabled={submitting}>{submitting ? 'Saving...' : !isOnline ? '📥 Save Offline' : form.payment_type === 'credit' ? '📒 Credit Sale' : '💵 Save Sale'}</button><button type="button" onClick={() => { setShowModal(false); resetForm(); }} className="entry-cancel-button h-11 rounded-xl border border-gray-200 px-4 text-sm font-semibold text-slate-600">Cancel</button></div></div>
        </aside>
      </div>
