      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 4 }}>
            <div style={{ color: '#f59e0b', fontSize: 13, fontWeight: 600 }}>Total Spent: ₹{total.toFixed(2)}</div>
            {totalGST > 0 && <div style={{ color: '#6366f1', fontSize: 13, fontWeight: 600 }}>GST Input Credit: ₹{totalGST.toFixed(2)}</div>}
          </div>
        </div>
        ...
        : purchases.length === 0 ? (
          ...
          <>
            <div className="table-container hidden-xs">
              <table>
                <thead><tr><th>Bill No.</th><th>Product</th><th>Qty</th><th>Taxable</th><th>GST</th><th>Total</th><th>Date</th><th>Actions</th></tr></thead>
                <tbody>
                  {purchases.map(p => (
                    <tr key={p._id}>
                      ...
                      <td>
                        ...
                        {p.supplier_name && <div style={{ fontSize: 11, color: '#9ca3af' }}>From: {p.supplier_name}</div>}
                      </td>
                      <td>{p.quantity}</td>
                      <td>₹{p.taxable_amount?.toFixed(2)}</td>
                      <td>
                        {p.gst_rate > 0
                          ? <span style={{ background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{p.gst_rate}% • ₹{p.total_gst?.toFixed(2)}</span>
              ...
              {purchases.map(p => (
                <div key={p._id} className="card">
                  ...
                  <div style={{ display: 'flex', gap: 12, marginBottom: 10, flexWrap: 'wrap' }}>
                    ...
                    <div><div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600 }}>GST</div><div style={{ fontWeight: 600 }}>{p.gst_rate > 0 ? `${p.gst_rate}% • ₹${p.total_gst?.toFixed(2)}` : 'None'}</div></div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {gstPreview && (
                <div style={{ background: gstPreview.gst_rate > 0 ? '#fffbeb' : '#f0fdf4', border: `1px solid ${gstPreview.gst_rate > 0 ? '#fde68a' : '#bbf7d0'}`, borderRadius: 10, padding: '12px 14px', marginBottom: 16, fontSize: 13 }}>
                  <div style={{ fontWeight: 700, color: gstPreview.gst_rate > 0 ? '#92400e' : '#059669', marginBottom: 6 }}>Bill Preview</div>
              ...
              <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 14, marginBottom: 14 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 }}>Supplier Details (Optional)</div>
                <div className="form-group">
                  <label className="form-label">Supplier Name</label>
                  <input className="form-input" placeholder="Supplier name" value={form.supplier_name} onChange={e => setForm({ ...form, supplier_name: e.target.value })} />
                </div>
                <div className="grid-2">
                  <div className="form-group">
                    <label className="form-label">Supplier GSTIN</label>
                    <input className="form-input" placeholder="GSTIN (if any)" value={form.supplier_gstin} onChange={e => setForm({ ...form, supplier_gstin: e.target.value })} />
                  </div>
                  <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="Address" value={form.supplier_address} onChange={e => setForm({ ...form, supplier_address: e.target.value })} />
                </div>
                <div className="form-group">
