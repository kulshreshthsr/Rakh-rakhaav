...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (!user?.name) return;
    const timer = setTimeout(() => setProfileForm({ name: user.name }), 0);
        ...
        <Field label="Current Password" required>
          <div className="relative">
            <input
              ...
              required
            />
            <EyeButton show={showPw.current} onToggle={() => setShowPw((p) => ({ ...p, current: !p.current }))} />
          </div>
        </Field>
        <Field label="New Password" required>
          <div className="relative">
            <input
              ...
              required
            />
            <EyeButton show={showPw.newPwd} onToggle={() => setShowPw((p) => ({ ...p, newPwd: !p.newPwd }))} />
          </div>
        </Field>
        <Field label="Confirm New Password" required>
          <div className="relative">
            <input
              ...
              required
            />
            <EyeButton show={showPw.confirm} onToggle={() => setShowPw((p) => ({ ...p, confirm: !p.confirm }))} />
          </div>
