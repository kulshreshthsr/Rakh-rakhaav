...
function UserDropdown({ onProfile, onLogout, extraItems }) {
  return (
    <div className="sidebar-user-menu premium-user-dropdown">
  ...
  return (
    <div className="app-shell-root">
      <div className={subscription?.isReadOnly ? 'shell-readonly-content' : ''}>
        <header className={`mobile-topbar premium-topbar${scrolled ? ' is-scrolled' : ''}`}>
          <div className="content-container premium-topbar-inner">
            <Link href="/dashboard" className="premium-brand-lockup">
              <div className="premium-brand-kicker">{t('workspace')}</div>
              <div className="mobile-brand-title premium-brand-title">रखरखाव</div>
            </Link>

            <nav className="premium-topnav" aria-label="Primary">
              {translatedNav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`premium-nav-link${pathname === item.href ? ' is-active' : ''}`}
                >
                  <Glyph name={item.key} size={16} />
                  <span>{item.shortLabel}</span>
                </Link>
              ))}
            </nav>

            <div className="mobile-topbar-actions premium-topbar-actions">
              <Link href="/pricing" className={`top-upgrade-chip${subscription?.isPro ? ' is-manage' : ' is-shining'}`}>
                {upgradeLabel}
              </Link>

              <div ref={dropdownRef} style={{ position: 'relative' }}>
                <button type="button" className="premium-avatar-button" onClick={() => setDropdownOpen((v) => !v)}>
                  <span className="premium-avatar-ring">{initial}</span>
                </button>
                {dropdownOpen ? (
                  <UserDropdown
